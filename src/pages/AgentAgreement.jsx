...
export default function AgentAgreement() {
  ...
  useEffect(() => {
    const loadAgreement = async () => {
      try {
        const ref = doc(db, "agreements", "agent_agreement");
        const snap = await getDoc(ref);
        if (snap.exists()) {
          const data = snap.data();
          setHtml(data?.html || DEFAULT_HTML);
          setLastUpdated(toJsDate(data?.last_updated) || new Date());
        } else {
          // Fallback to default content when no Firestore doc exists
          setHtml(DEFAULT_HTML);
          setLastUpdated(new Date());
        }
      } catch (e) {
        console.error("Failed to load agent agreement:", e);
        setHtml(DEFAULT_HTML);
        setLastUpdated(new Date());
      } finally {
        setLoading(false);
      ...
      <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="bg-white p-8 sm:p-10 rounded-lg shadow-md">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">
            Professional Agent Agreement
          </h1>
          <p className="text-sm text-gray-500 mb-6">
            Last Updated: {lastUpdated ? lastUpdated.toLocaleDateString() : ""}
          </p>

          {/* We assume `html` stored in Firestore is sanitized HTML. */}
          <div
