  ...
  >
    ...
    <div className="p-5 flex-grow flex flex-col">
      ...
      <div className="text-sm text-gray-500 mb-4">
        {format(toJsDate(event.start), 'EEEE, MMMM dd, yyyy')}
      </div>

      {!isPast && toJsDate(event.start) > new Date() && (
        <div className="my-4">
          <SimpleCountdown targetDate={toJsDate(event.start)} />
        </div>
      )}
...
export default function FairAndEvents() {
    ...
    const loadEvents = async () => {
      ...
      try {
        ...
        const items = snap.docs.map((d) => {
          ...
          return {
            id: d.id,
            event_id: data.event_id || d.id, // ensure we always have an event_id for links
            ...data,
          };
        });
        setEvents(items);
  ...
  // Sort by sort_order then by start date
  ...
  const upcomingEvents = sortedEvents.filter((event) => {
    ...
    const isArchived = event.archive_at && toJsDate(event.archive_at) < now;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 text-center">
          ...
          <p className="text-gray-600 text-lg">
            Connect with top institutions and education professionals at our
            exclusive events
          </p>
        </div>
