  ...
  const rows = data.map((row) => {
    return headers
      .map((header) => {
        ...
        const escaped = ("" + (cell == null ? "" : cell)).replace(/"/g, '""');
        return `"${escaped}"`;
      })
      .join(",");
  });
...
export default function AdminReports() {
  ...
  // Small helper to check if current user is admin
  const checkIsAdmin = async (firebaseUser) => {
    ...
    try {
      ...
      // ignore and try user doc fallback
    }
    // Fallback: check users/{uid}.user_type === 'admin'
    try {
      const usersSnap = await getDocs(collection(db, "users"));
      const docForUser = usersSnap.docs.find((d) => d.id === firebaseUser.uid);
      const data = docForUser?.data();
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        ...
        // Pull collections (adjust names if your schema differs)
        const [
          usersSnap,
          paymentsSnap,
          casesSnap,
          ...
          getDocs(collection(db, "tutoringSessions")),
          getDocs(collection(db, "marketplaceOrders")),
        ...
        const tutoringSessions = tutoringSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const marketplaceOrders = marketplaceSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const schoolReservations = reservationsSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        ...
        const revenueByType = successfulPayments.reduce((acc, p) => {
          ...
          acc[type] = (acc[type] || 0) + (Number(p.amount_usd) || 0);
          return acc;
        }, {});
        ...
        setReportData({
          ...
          revenue: {
            ...
            packages:
              (revenueByType.visa_package_purchase || 0) +
              (revenueByType.tutor_subscription || 0) +
              (revenueByType.agent_subscription || 0),
  ...
  const handleExportUserData = () => {
    ...
    try {
      ...
      downloadCSV(
        csvData,
        `greenpass-report-${new Date().toISOString().split("T")[0]}.csv`
      );
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          {reportData && (
            <Button onClick={handleExportUserData} variant="outline">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid md:grid-cols-4 gap-6">
              ...
              <MetricCard
                ...
                value={
                  reportData.platform.tutoringSessions +
                  reportData.platform.marketplaceOrders +
                  reportData.platform.schoolReservations
                }
                icon={BarChart3}
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-4 bg-purple-50 rounded-lg">
                      ...
                      <span className="font-semibold">
                        {reportData.platform.visaCases.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-4 bg-blue-50 rounded-lg">
                      <span>Tutoring Sessions</span>
                      <span className="font-semibold">
                        {reportData.platform.tutoringSessions.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-4 bg-green-50 rounded-lg">
                      <span>School Reservations</span>
                      <span className="font-semibold">
                        {reportData.platform.schoolReservations.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-4 bg-orange-50 rounded-lg">
                      <span>Marketplace Orders</span>
                      <span className="font-semibold">
                        {reportData.platform.marketplaceOrders.toLocaleString()}
                      </span>
                    </div>
                  </div>
                </CardContent>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-4 bg-green-50 rounded-lg">
                      ...
                      <span className="font-semibold">
                        {reportData.revenue.events.toLocaleString("en-US", {
                          style: "currency",
                          currency: "USD",
                        })}
                      </span>
                    ...
                    <div className="flex justify-between items-center p-4 bg-blue-50 rounded-lg">
                      ...
                      <span className="font-semibold">
                        {reportData.revenue.packages.toLocaleString("en-US", {
                          style: "currency",
                          currency: "USD",
                        })}
                      </span>
                    ...
                    <div className="flex justify-between items-center p-4 bg-orange-50 rounded-lg">
                      ...
                      <span className="font-semibold">
                        {reportData.revenue.marketplace.toLocaleString("en-US", {
                          style: "currency",
                          currency: "USD",
                        })}
                      </span>
          ...
          <TabsContent value="users" className="space-y-6">
            <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-4">
              <MetricCard title="Total Users" value={reportData.users.total} icon={Users} color="text-gray-600" />
              <MetricCard title="Students" value={reportData.users.students} icon={Users} color="text-blue-600" />
              <MetricCard title="Agents" value={reportData.users.agents} icon={Users} color="text-green-600" />
              <MetricCard title="Tutors" value={reportData.users.tutors} icon={Users} color="text-purple-600" />
              <MetricCard title="Vendors" value={reportData.users.vendors} icon={Users} color="text-orange-600" />
              <MetricCard title="Schools" value={reportData.users.schools} icon={Users} color="text-red-600" />
            </div>
          </TabsContent>
