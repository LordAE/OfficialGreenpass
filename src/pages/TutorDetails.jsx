    ...
    <Card className="bg-white/95 backdrop-blur-sm hover:shadow-lg transition-all duration-300 border rounded-xl flex flex-col">
      <CardContent className="p-6 flex flex-col flex-grow">
        <div className="text-center mb-4">
          ...
          {pkg.target_user && <p className="text-sm text-gray-500">{pkg.target_user}</p>}
          {pkg.price_display && <p className="text-2xl font-bold text-green-600 my-2">{pkg.price_display}</p>}
        </div>
...
export default function TutorDetails() {
  ...
  useEffect(() => {
    const fetchTutorDetails = async () => {
      ...
      try {
        // Firestore: fetch by document ID
        const tutorRecord = await Tutor.get(tutorId);
        if (!tutorRecord) {
        ...
        // Load packages (no filters needed here)
        try {
  ...
  // Prefer tutor doc fields (since we don't fetch a separate user profile in Firebase)
  ...
  return (
    <>
      <BookingModal open={showBookingModal} onOpenChange={setShowBookingModal} tutor={tutor} tutorUser={null} />
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-6 mb-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-2xl font-bold text-green-600">${tutor.hourly_rate || 25}/hr</span>
                    </div>

                    <div className="flex items-center gap-2">
                      <Clock className="w-5 h-5 text-gray-500" />
                      <span className="text-gray-600">{tutor.experience_years || 0} years experience</span>
                    </div>
                  </div>
          ...
          <div className="mt-12">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {studentPackages.map((pkg) => (
                <StudentPackageCard key={pkg.id || pkg.name} pkg={pkg} onSelect={handleSelectStudentPackage} />
              ))}
            </div>
          </div>
        </div>
