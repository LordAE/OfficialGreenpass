...
const mapDocToPost = (docSnap) => {
  ...
  return {
    ...
    published: d.published, // may be undefined; handled below
  };
};
...
export default function Blog() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        // Simple read (no indexes required). We’ll sort client-side by created_at/created_date.
        const snap = await getDocs(collection(db, 'posts'), limit(100));
        let items = snap.docs.map(mapDocToPost);

        // If a `published` flag exists, only show published; otherwise show all.
        const hasPublishedFlag = items.some((p) => typeof p.published === 'boolean');
        if (hasPublishedFlag) {
          items = items.filter((p) => p.published === true);
        }

        // Sort newest first by created_at (timestamp/number/iso), fallback to updated_at
        items.sort(
          (a, b) =>
