...
const sanitizeHomeContent = (loaded = {}) => {
  ...
  return {
    ...
    features_section: Array.isArray(loaded.features_section) ? loaded.features_section : [],
    testimonials_section: Array.isArray(loaded.testimonials_section) ? loaded.testimonials_section : [],
    stats_section: Array.isArray(loaded.stats_section) ? loaded.stats_section : [],
...
========================= */
const Hero = ({ content }) => {
  const h = content?.hero_section || {};
  const hasAnything = !!(h.title || h.subtitle || h.image_url || h.video_url);
  if (!hasAnything) return null;
  // ...render only what's provided (no default headline/image)...
};

const Features = ({ features }) => {
  const items = Array.isArray(features) ? features : [];
  if (items.length === 0) return null;
  // ...map items (no default titles/images)...
};
...
const FinalCTA = ({ ctaContent }) => {
  const c = ctaContent || {};
  const hasAnything = !!(c.title || c.subtitle || c.description || c.primary_button_text || c.secondary_button_text);
  if (!hasAnything) return null;
