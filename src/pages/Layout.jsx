...
const buildExploreForPartners = () => [
  ...
  {
    title: getText("vendorNetwork"),
    href: createPageUrl("Partner Vendors"),
    icon: Store,
    description: getText("joinVendorNetwork"),
  },
];
...
function HoverDropdown({ label, color = "green", items = [] }) {
  ...
  return (
    ...
    >
      <button
        className={`inline-flex items-center gap-1 rounded-lg px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-all ${
          open ? `bg-gray-100 ring-1 ${palette.ring} ${palette.text}` : ""
        }`}
        aria-expanded={open}
      >
        {label}
            ...
            <ul className="max-h-[70vh] overflow-auto p-2">
              {items.map((item) => (
                <li key={item.title}>
                  ...
                  >
                    <item.icon
                      className={`mt-0.5 h-5 w-5 ${palette.icon} ${palette.hover}`}
                    />
                    <div>
                      <div
                        className={`text-sm font-semibold text-gray-900 ${palette.hover}`}
                      >
                        {item.title}
                      </div>
                      {item.description && (
...
function CountriesMegaMenu() {
  ...
  const computeGeom = React.useCallback(() => {
    ...
    const btnBottom = (br?.bottom ?? 56) + 8;
    ...
    >
      <button
        ...
        className={`inline-flex items-center gap-1 rounded-lg px-4 py-2 text-sm font-semibold transition
          ${
            open
              ? "bg-gray-200 text-green-700 ring-1 ring-green-300"
          ...
          >
            <div className="grid grid-cols-12 gap-6 p-5">
              ...
              <div className="col-span-12 md:col-span-3 border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-4">
                <div className="text-xs font-extrabold text-pink-600 tracking-wide mb-2">
                  More Countries
                </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <HoverDropdown
              label={getText("forStudents")}
              color="green"
              items={students}
            />
            <HoverDropdown
              label={getText("forPartners")}
              color="purple"
              items={partners}
            />
            <HoverDropdown
              label={getText("quickLinks")}
              color="emerald"
              items={quick}
            />
          </div>
            ...
            >
              <div className="px-4 pt-4 pb-6 space-y-4">
                <div className="space-y-3">
                  <p className="text-xs uppercase font-bold text-blue-600 tracking-wider px-2">
                    {getText("forStudents")}
                  </p>
                  {students.map((link) => (
                    <Link
                ...
                <div className="space-y-3 border-t border-gray-200 pt-4">
                  <p className="text-xs uppercase font-bold text-purple-600 tracking-wider px-2">
                    {getText("forPartners")}
                  </p>
                  {partners.map((link) => (
                    <Link
                ...
                <div className="space-y-3 border-t border-gray-200 pt-4">
                  <p className="text-xs uppercase font-bold text-emerald-600 tracking-wider px-2">
                    {getText("quickLinks")}
                  </p>
                  {quick.map((link) => (
                    <Link
...
function buildDesktopNav(currentUser, hasReservation, latestReservationId) {
  ...
  const hasPurchasedPackage =
    currentUser?.purchased_packages &&
  ...
  switch ((currentUser?.user_type || "student").toLowerCase()) {
    ...
    case "student": {
      const studentNav = [
        ...
        {
          title: currentUser?.assigned_agent_id
            ? getText("myAgent")
            : getText("findAgent"),
          url: createPageUrl(
            currentUser?.assigned_agent_id ? "MyAgent" : "FindAgent"
          ),
          icon: UserCheck,
        },
        {
      ...
      // Only visible if they have at least one reservation
      if (hasReservation && latestReservationId) {
        studentNav.push({
          title: "Seat Reservations",
          url: `${createPageUrl(
            "ReservationStatus"
          )}?reservationId=${encodeURIComponent(latestReservationId)}`,
          icon: Ticket,
        });
      }
    ...
    case "school":
      return [
        ...baseItems,
        {
          ...
          url: createPageUrl("SchoolProfile"),
          icon: Building,
        },
        {
          title: getText("programs"),
          url: createPageUrl("SchoolPrograms"),
          icon: BookOpen,
        },
        {
          title: getText("leads"),
          url: createPageUrl("SchoolLeads"),
          icon: Users,
        },
      ];
    case "vendor":
...
function buildMobileNav(currentUser, hasReservation, latestReservationId) {
  ...
  switch ((currentUser?.user_type || "student").toLowerCase()) {
    ...
    case "student": {
      ...
      // Seat Reservations in mobile "More" menu
      if (hasReservation && latestReservationId) {
        more.splice(3, 0, {
          title: "Seat Reservations",
          url: `${createPageUrl(
    ...
    case "agent":
      return {
        main: [
          ...
          {
            title: getText("myStudentsShort"),
            url: createPageUrl("MyStudents"),
            icon: Users,
          },
          {
            title: getText("visaCasesShort"),
            url: createPageUrl("VisaCases"),
            icon: FileText,
          },
        ],
    ...
    case "vendor":
      return {
        main: [
          ...
          {
            title: getText("myServicesShort"),
            url: createPageUrl("MyServices"),
            icon: Store,
          },
          {
            title: getText("myOrdersShort"),
            url: createPageUrl("MyOrders"),
            icon: ShoppingCart,
          },
        ],
    ...
    case "school":
      return {
        main: [
          ...
          {
            title: getText("programsShort"),
            url: createPageUrl("SchoolPrograms"),
            icon: BookOpen,
          },
          {
            title: getText("leadsShort"),
            url: createPageUrl("SchoolLeads"),
            icon: Users,
          },
        ],
    ...
    case "admin":
      return {
        main: [
          ...
          {
            title: getText("userManagementShort"),
            url: createPageUrl("UserManagement"),
            icon: Users,
          },
          {
            title: getText("verificationsShort"),
            url: createPageUrl("Verification"),
            icon: UserCheck,
          },
        ],
        more: [
          ...
          {
            title: getText("institutionManagementShort"),
            url: createPageUrl("AdminInstitutions"),
            icon: Landmark,
          },
          {
...
export default function Layout() {
  ...
  const normalizeUser = React.useCallback((uid, data = {}, fbUser = {}) => {
    ...
    const onboarding_completed =
      data.onboarding_completed ?? data.onboardingComplete ?? false;
    return {
      id: uid,
      ...data,
      full_name,
      ...
      purchased_packages: Array.isArray(data.purchased_packages)
  ...
  // Check if current user has at least one reservation
  React.useEffect(() => {
    if (!currentUser?.id) {
  ...
  const navigationItems = React.useMemo(
    () => buildDesktopNav(currentUser, hasReservation, latestReservationId),
    [currentUser, hasReservation, latestReservationId]
  );
  const mobileNavigationItems = React.useMemo(
    () => buildMobileNav(currentUser, hasReservation, latestReservationId),
    [currentUser, hasReservation, latestReservationId]
  );
  ...
  React.useEffect(() => {
    if (
      ...
      location.pathname.toLowerCase().startsWith("/onboarding")
    ) {
