...
export default function EventForm({ event, onSave, onCancel }) {
  ...
  const [progress, setProgress] = useState({ cover: 0, gallery: 0, video: 0 });
  const [tempPreviews, setTempPreviews] = useState({ cover: null, gallery: [] });

  useEffect(() => {
    const initialTiers =
      event?.registration_tiers?.map((t) => ({ ...t, benefits: t.benefits?.join('\n') || '' })) || [];
  ...
  // Organized base path for this event's assets
  const storageBase = `events/${formData.event_id || event?.event_id || 'unassigned'}`;
  ...
  const handleImageUpload = async (e, type) => {
    ...
    if (type === 'cover') {
      setTempPreviews((p) => ({ ...p, cover: localPreview }));
    } else {
      setTempPreviews((p) => ({ ...p, gallery: [...p.gallery, localPreview] }));
    }

    setUploading(true);
    try {
      const sub = type === 'cover' ? 'cover' : 'gallery';
      const result = await UploadFile({
        file,
        path: `${storageBase}/${sub}`,
        onProgress: (pct) =>
          setProgress((p) => ({
            ...p,
            [type === 'cover' ? 'cover' : 'gallery']: pct,
          })),
      });

      const fileUrl = result?.file_url || result?.url;
      if (!fileUrl) throw new Error('UploadFile must return { file_url } or { url }.');

      // ✅ Console success line for your Firebase project logs
      console.log(`[EventForm] ${type} uploaded`, {
        url: fileUrl,
      ...
      if (type === 'cover') {
        ...
        setTempPreviews((p) => ({ ...p, cover: null }));
      } else {
        setFormData((prev) => ({
          ...prev,
          gallery_images: [...(prev.gallery_images || []), fileUrl],
        }));
        setTempPreviews((p) => ({ ...p, gallery: p.gallery.filter((u) => u !== localPreview) }));
      }
    } catch (error) {
      console.error('Upload error:', error);
      alert('Failed to upload image.');
      // Clean temp preview on failure
      if (type === 'cover') {
        setTempPreviews((p) => ({ ...p, cover: null }));
      } else {
        setTempPreviews((p) => ({ ...p, gallery: p.gallery.filter((u) => u !== localPreview) }));
      ...
      setUploading(false);
      setTimeout(
        () => setProgress((p) => ({ ...p, [type === 'cover' ? 'cover' : 'gallery']: 0 })),
        800
      );
      e.target.value = '';
      // Revoke memory after a short delay
  ...
  const handleVideoUpload = async (e) => {
    ...
    try {
      const result = await UploadFile({
        file,
        path: `${storageBase}/promo_video`,
        onProgress: (pct) => setProgress((p) => ({ ...p, video: pct })),
      });
      ...
      console.log('[EventForm] video uploaded', {
        url: fileUrl,
        storage_path: result?.storage_path,
        bytes: result?.size,
        content_type: result?.content_type,
      ...
      setFormData((prev) => ({ ...prev, promo_video: fileUrl }));
    } catch (error) {
      console.error('Video upload error:', error);
      alert('Failed to upload video.');
    } finally {
      setUploading(false);
      setTimeout(() => setProgress((p) => ({ ...p, video: 0 })), 800);
      e.target.value = '';
    }
  };
  ...
  const handleSubmit = () => {
    ...
    // Keep fields consistent based on selected videoMode
    if (videoMode !== 'upload') dataToSave.promo_video = null;
    if (videoMode !== 'youtube') dataToSave.promo_video_url = '';

    // Remove flattened fields from top-level
            ...
            <div key={tier.key || index} className="p-4 border rounded-md relative space-y-3 bg-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => handleTierChange(index, 'price_usd', parseFloat(e.target.value))}
                    placeholder="500"
                    required
                  />
                </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder={`Premium booth location\nMarketing materials included\nVIP networking access`}
                  rows={4}
                />
              </div>
            </div>
      ...
      <div className="space-y-4 p-4 border rounded-lg">
        ...
        <div>
          ...
          <div className="mt-2">
            ...
            >
              ...
              {uploading && progress.cover > 0 ? `Uploading… ${progress.cover}%` : 'Upload Cover Image'}
            </label>
          </div>
          {(formData.cover_image || tempPreviews.cover) && (
            <div className="mt-2">
              <img
                src={formData.cover_image || tempPreviews.cover}
                alt="Cover"
                className="w-32 h-20 object-cover rounded"
                onError={(e) => {
                  // fallback to local preview if the URL fails
                  if (tempPreviews.cover) e.currentTarget.src = tempPreviews.cover;
                }}
              />
            </div>
          )}
        ...
        <div>
          ...
          <div className="mt-2">
            ...
            >
              <Upload className="w-4 h-4" />
              {uploading && progress.gallery > 0 ? `Uploading… ${progress.gallery}%` : 'Add Gallery Image'}
            </label>
          </div>

          {(formData.gallery_images?.length > 0 || tempPreviews.gallery.length > 0) && (
            <div className="mt-2 grid grid-cols-4 gap-2">
              ...
              {/* In-flight previews */}
              {tempPreviews.gallery.map((url, i) => (
                <div key={`temp-${i}`} className="relative opacity-80">
                  <img src={url} alt="Uploading…" className="w-full h-20 object-cover rounded" />
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          <div className="flex gap-2 mb-3">
            <Button type="button" variant={videoMode === 'none' ? 'default' : 'outline'} onClick={() => setVideoMode('none')}>
              None
            </Button>
            <Button type="button" variant={videoMode === 'upload' ? 'default' : 'outline'} onClick={() => setVideoMode('upload')}>
              Upload Video
            </Button>
            <Button type="button" variant={videoMode === 'youtube' ? 'default' : 'outline'} onClick={() => setVideoMode('youtube')}>
              YouTube Link
            </Button>
          </div>
          ...
          {videoMode === 'upload' && (
            <div>
              ...
              >
                <Upload className="w-4 h-4" />
                {uploading && progress.video > 0 ? `Uploading… ${progress.video}%` : 'Upload Video'}
              </label>
        ...
        <div>
          ...
          <Input
            ...
            value={formData.media_attribution || ''}
            onChange={handleChange}
            placeholder="Photo credits, video source, etc."
          />
        </div>
      </div>
